...
namespace Dcat\Admin\Extension\Crontab\Http\Commands;
...
class AutoTask extends Command
{
    ...
    {
        // 筛选未过期且未完成的任务
        $crontab_list = Crontabs::where(['status' => 'normal'])->orderBy('weigh','desc')->orderBy('id','desc')->get();
        if (!$crontab_list) {
            return null;
        }
        $time = time();
